...
type value =
  ...
  | U
  | E
  | C of value * value * commands * ((string * value) list)
...
let rec check_error st =
  match st with
  | [] -> false
  | h::t -> if h = E then true else check_error t
...
let rec interp coms stack env =
  ...
  | (TryWith (coms1, coms2)) :: coms, _ ->
    (match interp coms1 stack env with
     | Ok curr_stack -> if (check_error curr_stack) then interp coms2 stack env 
       else interp coms ((get_top curr_stack)::stack) env
     | Exit stack -> Exit stack)
  | (FunEndFun (fname, arg, coms)) :: rem_coms, _ ->
    interp (Bnd::rem_coms) (fname::(C(fname, arg, coms, env)::stack)) env
  | Call :: coms, x :: y :: stack ->
    (
      ...
      | Some x1, Some y1 -> 
        (
          match y1 with
          | C (fname, arg, fun_coms, fun_env) -> 
            (
              let arg_name = get_name arg in
              (
                match arg_name with
                | Some fun_arg -> 
                  (
                    match interp (Bnd::Pop::fun_coms) (y::(C(y, arg, fun_coms, ((fun_arg, x1)::fun_env)))::stack) ((fun_arg, x1)::fun_env) with
                    | Ok (sv :: _) -> interp coms (sv::stack) env
                    | Exit (stack) -> Exit stack
                    | error -> interp coms (E :: stack) env
                  )
                | _ -> interp coms (E :: x :: y :: stack) env
              )
            )
          | _ -> interp coms (E :: x :: y :: stack) env
        )
...
(*let _= interpreter "part3-public/input/input24.txt" "part3-public/myoutput/myoutput24.txt"*)
